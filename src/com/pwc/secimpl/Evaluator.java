...
public class Evaluator {
	...
	public SecOperand evaluate(SecOperand opr1, SecOperand opr2, SecOperator op) throws RuntimeException {
		...
		if (opr1.getSecLevel().getLevel() < opr2.getSecLevel().getLevel()
				|| opr1.getSecLevel().getLevel() > opr2.getSecLevel().getLevel())
			throw new UnsupportedOperationException("Operation Not Allowed");
		else {

			if (TypeSystem.getType(opr1).equalsIgnoreCase("SecInt")) {
				s = new SecInt(0);
				((SecInt) s).setSecLevel(opr1.getSecLevel());
				((SecInt) s)
						.setVal(evalOp(((SecInt) opr1).getVal(), ((SecInt) opr2).getVal(), (char) op.getOperator()));
			}
			if (TypeSystem.getType(opr1).equalsIgnoreCase("SecFloat")) {
				s = new SecFloat(0);
				((SecFloat) s).setSecLevel(opr1.getSecLevel());
				((SecFloat) s).setVal(
						evalOp(((SecFloat) opr1).getVal(), ((SecFloat) opr2).getVal(), (char) op.getOperator()));
			}
		}
		return s;
	}
